...
public class RoutConfig {
    ...
    public RouteLocator gatewayRoutes(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                ).route(r -> r.path("/ss/**") //라우터 등록 => JWT 토큰 발급 및 로그인 처리 수행하는 서비스
                        .filters(
                                // URL별 독립적으로 저장 항목을 추가할 경우 정의함
                                f -> f.addRequestHeader("ss-request", "ss-request-value")
                                        .addResponseHeader("ss-response", "ss-response-value")
                        )
                        .uri("http://localhost:15000") // 연결될 서버 주소
                )
                .build();
    ...
    RouterFunction staticResourceLocator() {
        return RouterFunctions.resources("/css/**", new ClassPathResource("static/**"));
